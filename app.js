...
function renderAdminPanel(tab = "overview") {
  const roleRows = [
    ["Super Admin", "All permissions", "Can create roles, assign access, edit all site sections, install plugins"],
    ["Admin", "Operations", "Can approve users, manage calendar, view analytics, publish approved media"],
    ["Pastor", "Ministry", "Can manage sermons, prayer requests, pastoral content, and member follow-up"],
    ["Media Team", "Media", "Can upload streams, edit archive bundles, captions, languages, and thumbnails"],
    ["Finance", "Payments", "Can review donations, convention payments, receipts, and finance exports"],
    ["Editor", "Website", "Can edit assigned pages and content blocks only"],
    ["Member", "Community", "Can register, view allowed resources, donate, and join events"],
  ];
  const permissionRows = [
    ["Live Player", "Super Admin, Admin, Media Team", "Quality, languages, DVR, stream routing"],
    ["Calendar", "Super Admin, Admin, Pastor, Editor", "Events, resources, previous-event archive links"],
    ["Archive", "Super Admin, Admin, Media Team, Editor", "4-in-1 bundles, PDF/EPUB content, permissions"],
    ["Analytics", "Super Admin, Admin", "User matrix, downloads, page journeys, session intelligence"],
    ["Payments", "Super Admin, Finance", "Donation funds, provider settings, payment reconciliation"],
    ["Website Sections", "Super Admin, Editor", "Hero, registration, giving, landing copy, media labels"],
  ];
  const pluginRows = [
    ["OpenAI", "AI chatbot, search, summaries", "Connected"],
    ["Cloudflare Stream / Mux", "Live video, DVR, adaptive quality", "Ready"],
    ["Stripe / Paystack / Flutterwave", "Donations and convention payments", "Configurable"],
    ["Telegram / Slack / Email", "Approvals and alerts", "Connected"],
    ["S3 / R2 Storage", "Archive files and uploads", "Ready"],
    ["OpenSearch / Vector DB", "PDF, EPUB, transcript retrieval", "Planned"],
  ];
  const editorRows = [
    ["Home hero", "Editor", "Draft", "Title, call-to-action, media background"],
    ["Live worship console", "Media Team", "Published", "Streams, languages, quality presets"],
    ["Registration forms", "Admin", "Published", "Fields, access rules, approval route"],
    ["Archive page", "Media Team", "Draft", "Bundle labels, file visibility, featured resource"],
    ["Giving page", "Finance", "Published", "Funds, payment provider, receipt text"],
  ];
  const views = {
    overview: {
      eyebrow: "Overview",
    ...
    access: {
      ...
      title: "Role Classes and Feature Access",
      body: "Assign every user to a class and grant only the features they should see or edit. Super Admin owns role creation, permission inheritance, and emergency lockout.",
      actions: ["Create role class", "Assign feature access", "Lock sensitive pages", "Audit permissions"],
      table: `
        <div class="access-matrix">
          <div class="matrix-row matrix-head"><span>User class</span><span>Access scope</span><span>Editable features</span></div>
          ${roleRows.map((row) => `<div class="matrix-row"><span>${row[0]}</span><span>${row[1]}</span><span>${row[2]}</span></div>`).join("")}
        </div>
        <div class="access-matrix feature-matrix">
          <div class="matrix-row matrix-head"><span>Feature</span><span>Allowed classes</span><span>Permission detail</span></div>
          ${permissionRows.map((row) => `<div class="matrix-row"><span>${row[0]}</span><span>${row[1]}</span><span>${row[2]}</span></div>`).join("")}
        </div>
      `,
    },
    plugins: {
      eyebrow: "Integration hub",
      title: "APIs and Plugin Registry",
      body: "Install, configure, test, and audit external APIs or plugins without changing the core website. Each plugin declares permissions, webhooks, and fallback behavior.",
      actions: ["Install plugin", "Test API key", "Rotate secret", "View webhook logs"],
      table: `
        <div class="plugin-registry">
          ${pluginRows.map((row) => `<article><strong>${row[0]}</strong><span>${row[1]}</span><em>${row[2]}</em></article>`).join("")}
        </div>
      `,
    },
    editor: {
      eyebrow: "Permissioned editing",
      title: "Website Content Editor",
      body: "Admins and editors can update assigned website sections only. Every change creates a draft, approval request, version history, and rollback point.",
      actions: ["Create draft", "Request approval", "Preview section", "Rollback version"],
      table: `
        <div class="editor-matrix">
          <div class="matrix-row matrix-head"><span>Section</span><span>Editable by</span><span>Status</span><span>Content blocks</span></div>
          ${editorRows.map((row) => `<div class="matrix-row"><span>${row[0]}</span><span>${row[1]}</span><span>${row[2]}</span><span>${row[3]}</span></div>`).join("")}
        </div>
      `,
    },
  };

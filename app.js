...
async function hydrateContentFromBackend() {
  const [liveResources, liveEvents, liveBundles, pageText] = await Promise.all([
    fetchBackendJson("/api/content/resources"),
    fetchBackendJson("/api/content/calendar-events"),
    fetchBackendJson("/api/content/archive-bundles"),
    fetchBackendJson("/api/content/page-text"),
  ]);

  let changed = false;

  if (Array.isArray(liveResources) && liveResources.length) {
    resources.splice(0, resources.length, ...liveResources);
    changed = true;
  }
  if (Array.isArray(liveEvents) && liveEvents.length) {
    calendarEvents.splice(0, calendarEvents.length, ...liveEvents);
    changed = true;
  }
  if (Array.isArray(liveBundles) && liveBundles.length) {
    archiveBundles.splice(0, archiveBundles.length, ...liveBundles);
    changed = true;
  }

  if (pageText && pageText.hero) {
    applyHeroText(pageText.hero);
  }

  if (changed) {
    searchResources($("#searchInput") ? $("#searchInput").value : "");
    renderArchive();
    buildCalendar();
  }
}
...
function applyHeroText(hero) {
  ...
  const eyebrow = heroSection.querySelector(".hero-copy .eyebrow");
  const title = heroSection.querySelector(".hero-copy h1");
  const body = heroSection.querySelector(".hero-copy p:not(.eyebrow)");
  const primary = heroSection.querySelector(".primary-action");
  const secondary = heroSection.querySelector(".secondary-action");
  const statEls = heroSection.querySelectorAll(".hero-stats > div");

  if (eyebrow && hero.eyebrow) eyebrow.textContent = hero.eyebrow;
  if (title && hero.title) title.textContent = hero.title;
  if (body && hero.body) body.textContent = hero.body;
  if (primary) {
    if (hero.primaryActionLabel) primary.textContent = hero.primaryActionLabel;
    if (hero.primaryActionHref) primary.setAttribute("href", hero.primaryActionHref);
  }
  if (secondary) {
    if (hero.secondaryActionLabel) secondary.textContent = hero.secondaryActionLabel;
    if (hero.secondaryActionHref) secondary.setAttribute("href", hero.secondaryActionHref);
  }

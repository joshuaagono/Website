...
async function handleApi(req, res, url) {
  ...
  if (req.method === "GET" && url.pathname === "/api/admin/site-sections") {
    return sendJson(res, 200, db.siteSections || []);
  }

  if (req.method === "GET" && url.pathname === "/api/admin/plugins") {
    return sendJson(res, 200, db.plugins || []);
  }
  ...
  if (req.method === "POST" && url.pathname === "/api/admin/assign-role") {
    const body = await readBody(req);
    const user = db.users.find((item) => item.id === body.userId);
    const role = (db.roles || []).find((item) => item.name === body.accessClass || item.id === body.roleId);
    if (!user) return sendJson(res, 404, { error: "User not found" });
    if (!role) return sendJson(res, 404, { error: "Role not found" });
    user.accessClass = role.name;
    user.role = role.name.toLowerCase().replace(/\s+/g, "_");
    db.adminAuditLog.push({ id: `AUD-${Date.now()}`, action: `Assigned ${role.name} to ${user.id}`, actor: "super_admin", createdAt: new Date().toISOString() });
    await writeDb(db);
